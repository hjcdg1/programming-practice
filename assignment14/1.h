...
board* look_board (savenode* head)
{
...
void insert2 (savescore* head, int item)
{
        if(head->next == NULL){
                head->next = (savescore*)malloc(sizeof(savescore));
                head->next->d = item;
                head->next->next = NULL;
        }
        else
                insert2(head->next, item);
}
